...
OBJ_DIR   = ./objects
EXTRA_SRC =
EXCLUDE_FILES = ./tryServer.c
# src/record_pool_impl_snippet.h src/container_impl_snippet.cpp src/container_impl_snippet.h


#DEFS += -DUFTCORE_VER=\"$(UFTCORE_VER)\"

SUFFIX       = c cpp cc cxx
TARGET       = ./a.out
#.$(UFTCORE_VER)
#TARGET_TYPE  := ar/app/so
TARGET_TYPE  := app
...
$(TARGET): $(all_objs)
	$(LD) $(LDFLAGS) $(all_objs) $(EXTRA_LIBS) -o $@
endif
